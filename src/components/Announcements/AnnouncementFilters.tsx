...
const AnnouncementFilters = ({ scrollToTop }: { scrollToTop: () => void }) => {
  ...
  const {
    ...
    queryKey: ['phasesfilter', villageData?.id], // include ID in cache key
    queryFn: () => fetchSubdivisionPhases(villageData?.id), // pass it here
    enabled: !!villageData?.id, // only run if villageId exists
  });
